...
export default function Settings() {
  ...
  return (
    <div className="space-y-6 fade-up max-w-5xl" data-testid="settings-page">
      ...
      <div className="panel p-6">
        <div className="flex items-center gap-2 mb-4">
          <SettingsIcon size={16} className="text-[var(--accent-2)]" />
          <div className="section-title">Watched symbols</div>
        </div>
        <div className="space-y-4">
          {Object.entries(ALL_SYMBOLS).map(([market, syms]) => (
            <div key={market}>
              <div className="text-xs text-[var(--text-mute)] mb-2 font-semibold uppercase tracking-widest">{market}</div>
              <div className="flex gap-2 flex-wrap">
                {syms.map((s) => {
                  const on = cfg.symbols?.includes(s);
                  return (
                    <button key={s} onClick={() => toggleSym(s)} data-testid={`sym-toggle-${s}`}
                      className={`px-4 py-2 rounded-lg mono text-sm font-semibold transition border ${
                        on
                          ? "border-[var(--accent)] text-[var(--accent)] bg-[rgba(0,255,163,0.06)]"
                          : "border-[var(--line-2)] text-[var(--text-mute)]"
                      }`}>
                      {s}
                    </button>
                  );
      ...
      <div className="panel p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <div className="section-title">Signal filters</div>
        </div>
          ...
          {[
            ...
            <div key={f.k}>
              ...
              <input className="input mono" type="number" step={f.step || 1}
                value={cfg[f.k]} onChange={(e) => set(f.k, parseFloat(e.target.value) || 0)}
                data-testid={`cfg-${f.k}`} />
            </div>
          ))}
        </div>
        <div className="flex gap-6 mt-4">
          {[
            ...
            <label key={t.k} className="flex items-center gap-2 cursor-pointer text-sm">
              <input type="checkbox" checked={cfg[t.k]}
      ...
      <div className="panel p-6">
        <div className="flex items-center gap-2 mb-4">
          <Shield size={16} className="text-[var(--warn)]" />
