...
def api_backtest():
    ...
    strategy    = str(data.get("strategy",    "pdh_sweep")).lower()
...
def apply_config():
    ...
    Neither _auto_learn_silent nor any background task writes here automatically.
...
def api_walkforward():
    ...
    try:
        data        = request.get_json(force=True) or {}
        symbol      = (data.get("symbol") or "BTCUSDT").upper()
        wf_strategy = str(data.get("strategy", "pdh_sweep")).lower()
        ...
        def _wf_run_strategy(candle_slice, extra_cfg=None):
            ...
            if wf_strategy == "pdh_sweep":
                ...
                raise ValueError(f"Unknown wf_strategy: {wf_strategy}")

        if wf_strategy in ("pdh_sweep", "turn_of_month",
            ...
            raise ValueError(f"Unknown wf_strategy for grid setup: {wf_strategy}")
